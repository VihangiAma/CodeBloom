...
const PremiumServiceForm = ({ existingBooking, isEditMode, onDelete }) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const loggedInUser = {
          ...res.data.user,
          type: res.data.user.type || "supervisor", // fallback
        };
  ...
  // Submit handler - connect to Electrical API
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode) {
        // Update existing electrical service appointment (PUT)
        await axios.put(
          `http://localhost:5001/api/electrical/${formData.serviceID}`,
          formData,
          config
        );
        alert("Appointment updated successfully ✅");
      } else {
        // Create new electrical service appointment (POST)
        await axios.post(
          "http://localhost:5001/api/electrical/",
          formData,
          config
        );

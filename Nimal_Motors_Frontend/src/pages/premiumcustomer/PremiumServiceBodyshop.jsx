...
//       if (isEditMode) {
//         await axios.put(
//           `http://localhost:5001/api/bodyshopbooking/premium/${existingBooking.serviceID}`,
//           payload,
//           config
//         );
...
//       } else {
//         await axios.post(
//   "http://localhost:5001/api/bodyshopbooking/premium",
//   payload,
//   config
// );
...
//         {isEditMode ? "Update Booking" : "New Bodyshop Booking"}
//       </h2>
...
const PremiumServiceBodyshop = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:5001/api/bodyshop", payload, config);
      alert("Booking submitted ✅");
      setFormData({
        customerName: user?.fullName || "",
        vehicleType: "",
  ...
  return (
    <div className="max-w-xl mx-auto p-6 bg-white rounded-xl shadow-lg">
      <h2 className="text-2xl font-bold mb-4 text-center">New Bodyshop Booking</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block font-semibold">Customer Name</label>
          <input

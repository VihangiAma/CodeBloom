...
const InvoiceForm = ({ userRole, initialData = {}, onCancel, onSubmit }) => {
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === "item") {
      ...
      if (selected) {
        updated[index].pricePerUnit = selected.pricePerUnit;
      } else {
        updated[index].pricePerUnit = 0; // allow custom item
      }
    } else {
      updated[index][field] = value;
    }
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-10">
      <form
        onSubmit={handleSubmit}
        className="max-w-6xl mx-auto p-6 bg-white shadow-lg rounded-xl space-y-6 text-gray-800"
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {Object.entries(services).map(([key, value]) => (
            <div
              key={key}
              className="flex items-center bg-gray-50 p-3 border rounded"
            >
              <input
                type="checkbox"
                checked={value.selected}
        ...
        {section !== "service" && (
          <div>
            ...
            <table className="min-w-full mt-4 border text-sm text-gray-800 bg-white">
        ...
        <h5 className="text-lg text-red-600 font-medium">Repair Cost</h5>
        <input
          type="number"
          placeholder="Repair Cost"
          value={repairCost}
        ...
        <div className="mt-6">
          <table className="w-full text-left border text-gray-800 bg-white">
            <thead className="bg-gray-200 text-red-600">
              <tr>
                <th className="p-2 border">Service Cost</th>
                <th className="p-2 border">Inventory Cost</th>

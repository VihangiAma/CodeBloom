...
//     description: "", // For notes/street info
//     services: {
//       fullService: { selected: false},
//       bodyWash: { selected: false },
//       oilChange: { selected: false},
//       underbodyWash: { selected: false},
//       interiorVacuum: { selected: false},
//     },
//     totalCost: 0,
//   });

//   // Calculate total cost based on selected services and entered costs
//   useEffect(() => {
//     const cost = Object.values(formData.services).reduce(
//       (sum, service) => service.selected ? sum + Number(service.cost || 0) : sum,
//       0
//     );
...
//         {/* Services Selection with Cost */}
//         <div>
//           <label className="block mb-1 font-medium">Select Services & Enter Cost</label>
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
//                   onChange={(e) => handleServiceChange(e, key)}
//                   className="accent-blue-600"
//                 />
//                 <span className="capitalize w-32">{key.replace(/([A-Z])/g, " $100")}</span>
//                 <input
//                   type="number"
//                   placeholder="Cost"
//                   value={service.cost}
...
//             onChange={handleInputChange}
//             placeholder="e.g., Pickup from 123 Main St., or any other special instructions..."
//             className="w-full p-2 border rounded-lg"
//             rows={3}
//           />
//         </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {Object.entries(formData.services).map(([serviceKey, serviceData]) => (
              <div key={serviceKey} className="flex flex-col md:flex-row items-center gap-4 border p-4 rounded-lg">
                <label className="flex items-center space-x-2 w-full md:w-1/3">
                  <input
                    type="checkbox"
                    name={serviceKey}
                ...
                <div className="flex items-center space-x-2 w-full md:w-2/3">
                  <button
                    type="button"
                    onClick={() => handleServiceCostChange(serviceKey, serviceData.cost - 100)}
                  ...
                  <input
                    type="number"
                    name={`${serviceKey}_cost`}
                    value={serviceData.cost}
                    onChange={(e) => handleServiceCostChange(serviceKey, e.target.value)}
    ...
    <table className="w-full table-auto border border-gray-300 rounded-lg">
      <thead className="bg-gray-100">
        <tr>
          ...
          <th className="border px-4 py-2 text-right">Cost (LKR)</th>
        </tr>
      </thead>
      <tbody>
        {Object.entries(formData.services).map(([key, service]) =>
          service.selected ? (
            <tr key={key}>
              ...
              <td className="border px-4 py-2 text-right">{service.cost}</td>
            </tr>
          ) : null
        )}
      </tbody>
      <tfoot>
        <tr className="font-bold">
          ...
          <td className="border px-4 py-2 text-right">{formData.totalCost}</td>
        </tr>
      </tfoot>
    </table>
  </div>
